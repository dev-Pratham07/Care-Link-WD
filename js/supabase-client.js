...
const Auth = {
  ...
  async signInWithGoogle() {
    ...
    // specific handling for subdirectory deployments
...
const Database = {
  ...
  async addReview(userId, rating, comment) {
    ...
    try {
      ...
      if (error) {
        ...
        if (error.code === '23503') {
          console.log('Profile missing for review, attempting to create...');
  ...
  // Add health record (metadata only - file upload handled separately)
  async addHealthRecord(userId, recordData) {
    ...
    const { data, error } = await supabaseClient
        ...
        .insert({
            ...
            document_date: recordData.document_date || new Date(),
